  ...
  if [ "${USING_CUSTOM_GEN_PATH_IN_LOCAL_MODE}" != "true" ]; then
    ...
    for i in $(psql ${PSQL_OPTIONS} -v ON_ERROR_STOP=1 -q -A -t -c "${SQL_QUERY}"); do
      ...
      for ((j=1; j<=GEN_DATA_PARALLEL; j++)); do
        ...
        if [ "${LOG_DEBUG}" == "true" ]; then
          log_time "ssh -n ${EXT_HOST} \"bash -c 'cd ~/; ./generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_PATH} ${RNGSEED} > ${SEG_DATA_PATH}/${GEN_PATH_NAME}/logs/tpcds.generate_data.${CHILD}.log 2>&1 &'\""
        fi
        ssh -n ${EXT_HOST} "bash -c 'cd ~/; ./generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_PATH} ${RNGSEED} > ${SEG_DATA_PATH}/${GEN_PATH_NAME}/logs/tpcds.generate_data.${CHILD}.log 2>&1 &'" &
        CHILD=$((CHILD + 1))
      done
    done
  else
    ...
    for EXT_HOST in $(cat ${TPC_DS_DIR}/segment_hosts.txt); do
      ...
      for GEN_DATA_PATH in "${GEN_PATHS[@]}"; do
        for ((j=1; j<=GEN_DATA_PARALLEL; j++)); do
          ...
          if [ "${LOG_DEBUG}" == "true" ]; then
            log_time "ssh -n ${EXT_HOST} \"bash -c 'cd ~/; ./generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_SUBPATH} ${RNGSEED} > ${GEN_DATA_PATH}/${GEN_PATH_NAME}/logs/tpcds.generate.data.${CHILD}.log 2>&1 &'\""
          fi
          ssh -n ${EXT_HOST} "bash -c 'cd ~/; ./generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_SUBPATH} ${RNGSEED} > ${GEN_DATA_PATH}/${GEN_PATH_NAME}/logs/tpcds.generate.data.${CHILD}.log 2>&1 &'" &
          CHILD=$((CHILD + 1))
        done
      done
    done
  ...
  if [ "${RUN_MODEL}" != "local" ]; then      
    ...
    for GEN_DATA_PATH in "${GEN_PATHS[@]}"; do
      for ((j=1; j<=GEN_DATA_PARALLEL; j++)); do
        GEN_DATA_SUBPATH="${GEN_DATA_PATH}/${GEN_PATH_NAME}/${CHILD}"
        if [ "${LOG_DEBUG}" == "true" ]; then
          log_time "sh ${TPC_DS_DIR}/01_gen_data/generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_SUBPATH} ${RNGSEED} > ${GEN_DATA_PATH}/${GEN_PATH_NAME}/logs/tpcds.generate.data.${CHILD}.log 2>&1 &"
        fi
        sh ${TPC_DS_DIR}/01_gen_data/generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_SUBPATH} ${RNGSEED} > ${GEN_DATA_PATH}/${GEN_PATH_NAME}/logs/tpcds.generate.data.${CHILD}.log 2>&1 &
        CHILD=$((CHILD + 1))
      done
    done
